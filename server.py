...
RULE_PATTERNS = (
    RulePattern('packageManager', 'Package manager', 'Project rules', re.compile(r'\b(?:use|install(?: packages)? with)\s+(pnpm|npm|yarn|bun)\b', re.I), lambda match: match[1].lower(), lambda value: f'Use {value} for all package operations.'),
    RulePattern('testCommand', 'Test command', 'Project rules', re.compile(r'\b((?:pnpm|npm|yarn|bun)\s+test)\b', re.I), lambda match: match[1].lower(), lambda value: f'Run {value} before handoff.'),
    RulePattern('typescript', 'TypeScript', 'Project rules', re.compile(r'\b(prefer typescript|avoid\s+any)\b', re.I), lambda match: 'strict', lambda value: 'Prefer TypeScript; avoid any.'),
    RulePattern('commitStyle', 'Commit style', 'Delivery', re.compile(r'\b(conventional commits?|descriptive commit messages?)\b', re.I), lambda match: 'conventional' if 'conventional' in match[1].lower() else 'descriptive', lambda value: 'Follow conventional commits.' if value == 'conventional' else 'Write descriptive commit messages.'),
    RulePattern('secrets', 'Secrets policy', 'Delivery', re.compile(r'\b(never commit secrets|do not commit secrets|never commit.*env)\b', re.I), lambda match: 'protect', lambda value: 'Never commit secrets or local env files.'),
)
...
def compile_manifest(sources: list[dict[str, str]], resolutions: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    for source in sources:
        for raw_line in source.get('content', '').splitlines():
            ...
            assert match
            rule = {'key': pattern.key, 'title': pattern.title, 'section': pattern.section, 'value': pattern.value(match), 'render': pattern.render, 'text': line, 'source': source['name']}
    ...
    for key, rules in candidates.items():
        ...
        if len(unique) > 1:
            conflicts.append({'key': key, 'title': selected[-1]['title'], 'options': [{'value': rule['value'], 'text': rule['text'], 'source': rule['source']} for rule in unique], 'severity': 'high' if key == 'packageManager' else 'medium'})
    ...
    return {'manifest': manifest, 'conflicts': conflicts, 'directives': len(selected) + len(passthrough), 'inputTokens': input_tokens, 'outputTokens': output_tokens, 'reclaimed': max(0, round((1 - output_tokens / input_tokens) * 100)) if input_tokens else 0}
...
def context_bundle(manifest: str, task: str) -> dict[str, Any]:
    routes = {
        ...
        'api': ('Fix orders API timeout', ('project rules', 'delivery', 'context routing'), 'API contracts · database · test rules'),
    ...
    bundle = ''.join(block for block in blocks if block.startswith('# Lekgotla') or any(word in block.lower() for word in words)).strip() + '\n'
    tokens, full_tokens = estimate_tokens(bundle), estimate_tokens(manifest)
    return {'task': name, 'bundle': bundle, 'tokens': tokens, 'rules': len(re.findall(r'^-', bundle, re.M)), 'areas': areas, 'reduction': max(0, round((1 - tokens / full_tokens) * 100))}
    ...
    def do_POST(self) -> None:
        try:
            ...
            if self.path == '/api/compile':
                ...
                if not isinstance(sources, list) or any(not isinstance(item, dict) or not isinstance(item.get('name'), str) or not isinstance(item.get('content'), str) for item in sources):
                    return self.send_json(HTTPStatus.BAD_REQUEST, {'error': 'sources must be an array of { name, content }'})
